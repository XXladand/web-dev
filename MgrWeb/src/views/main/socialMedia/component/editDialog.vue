		...
		<el-dialog v-model="isShowDialog" :width="800" draggable="" :close-on-click-modal="false">
			...
			<el-form :model="ruleForm" ref="ruleFormRef" label-width="auto" :rules="rules">
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="社交媒体类型" prop="type">
							<el-select clearable v-model="ruleForm.type" placeholder="请选择类型">
								<el-option v-for="(item, index) in dl('sjmtlx')" :key="index" :value="Number(item.code)"
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="爬取类型" prop="type">
							<el-select clearable="" v-model="ruleForm.outSocialTaskType" placeholder="请选择爬取类型">
								<el-option v-for="(item, index) in dl('EnumOutSocialTaskType')" :key="index"
									:value="item.code" :label="`[${item.code}] ${item.value}`" />
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20"
						v-if="ruleForm.outSocialTaskType == 1">
						<el-form-item label="用户名" prop="userName">
							<el-input v-model="ruleForm.userName" placeholder="请输入用户名" maxlength="64" show-word-limit
								clearable />
						</el-form-item>
					</el-col>
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20"
						v-if="ruleForm.outSocialTaskType == 2">
						<el-form-item label="关键字" prop="keyword">
							<el-input v-model="ruleForm.keyword" placeholder="请输入关键字" maxlength="64" show-word-limit
								clearable />
						</el-form-item>
					</el-col>
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20"
						v-if="ruleForm.outSocialTaskType == 1 || ruleForm.outSocialTaskType == 2">
						<el-form-item label="开始时间" prop="sTime">
							<el-date-picker v-model="ruleForm.sTime" type="datetime" placeholder="开始时间" />
						</el-form-item>
					</el-col>
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20"
						v-if="ruleForm.outSocialTaskType == 1 || ruleForm.outSocialTaskType == 2">
						<el-form-item label="结束时间" prop="eTime">
							<el-date-picker v-model="ruleForm.eTime" type="datetime" placeholder="结束时间" />
						</el-form-item>
					</el-col>
				</el-row>
				<!-- <el-divider />
...
import { addSocialMedia, updateSocialMedia, detailSocialMedia } from "/@/api/main/socialMedia";
...
const rules = ref<FormRules>({
	...
	outSocialTaskType: [{ required: true, message: '请选择爬取类型！', trigger: 'blur', },],
});
...
const openDialog = async (row: any) => {
	...
	if (rowData.id)
		ruleForm.value = (await detailSocialMedia(rowData.id)).data.result;
	else {
...
const submit = async () => {
	...
	// 	}
	// }
	if (ruleForm.value.sTime && ruleForm.value.eTime) {
		if (ruleForm.value.sTime > ruleForm.value.eTime) {
			return ElMessage({
	...
	ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
		if (isValid) {
			...
			if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
				await addSocialMedia(values);
			} else {
				await updateSocialMedia(values);
			}
			closeDialog();
		} else {
			ElMessage({
