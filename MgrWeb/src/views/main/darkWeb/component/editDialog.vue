...
import { addDarkWeb, updateDarkWeb, detailDarkWeb } from "/@/api/main/darkWeb";
...
const openDialog = async (row: any) => {
	...
	if (rowData.id)
		ruleForm.value = (await detailDarkWeb(rowData.id)).data.result;
	else {
	...
	ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
		if (isValid) {
			...
			if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
				await addDarkWeb(values);
			} else {
				await updateDarkWeb(values);
			}
			closeDialog();
		} else {
			ElMessage({
