				...
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
						<el-form-item label="任务数据" prop="data">
							<el-input v-model="ruleForm.data" placeholder="请输入任务数据" type="textarea" maxlength="2000"
...
import { addCustomNetTask, updateCustomNetTask, detailCustomNetTask } from "/@/api/main/customNetTask";
...
const rules = ref<FormRules>({
	...
	deep: [{ required: true, message: '请选择爬取深度！', trigger: 'change', },],
});
...
const openDialog = async (row: any) => {
	...
	if (rowData.id)
		ruleForm.value = (await detailCustomNetTask(rowData.id)).data.result;
	else {
	...
	ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
		if (isValid) {
			...
			if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
				await addCustomNetTask(values);
			} else {
				await updateCustomNetTask(values);
			}
			closeDialog();
		} else {
			ElMessage({
